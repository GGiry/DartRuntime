...
public class BigInt implements Comparable<BigInt> {
  ...
  private BigInt(String val) {
    ...
    while (cursor < len &&
           Character.digit((int)val.charAt(cursor), 10) == 0)
        cursor++;
    if (cursor == len) {
        signum = 0;
        mag = ZERO.mag;
    ...
    int numBits = ((numDigits * 3402) >>> 10) + 1;
    ...
    magnitude[numWords - 1] = magVal = Integer.parseInt(group, 10);
    if (magVal < 0)
    ...
    while (cursor < len) {
        ...
        if (groupVal < 0)
            throw new NumberFormatException("Illegal digit");
        destructiveMulAdd(magnitude, 0x3b9aca00, groupVal);
    }
